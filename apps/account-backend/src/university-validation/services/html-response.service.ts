...
export class HtmlResponseService {
  ...
  async handleHtmlResponse(
    ...
    this.logger.debug('Analyzing response HTML for errors:', {
      ...
      containsCaptchaError: responseHtml.includes('segurança') || responseHtml.includes('captcha'),
      containsObrigatorio: responseHtml.includes('obrigatório'),
      containsInvalido: responseHtml.includes('inválido'),
      // Check for specific error patterns
      specificErrors: {
        'segurança está inválido': responseHtml.includes('segurança está inválido'),
        ...
        'Validade do documento': responseHtml.includes('Consulte a data de validade no documento.'),
      },
    });

    // Check for authentication code errors
    if (
      responseHtml.includes('O cÃ³digo de autenticação informado não consta em nossos registros') ||
      responseHtml.includes('O código de autenticação informado não consta em nossos registros')
    ) {
      return {
        success: false,
    ...
    return await this.handleUnexpectedResponse(responseHtml, sessionId, userId, session);
  }
  ...
  async handleUnexpectedResponse(
    ...
    try {
      this.logger.debug('Creating manual approval fallback due to unexpected server response', { sessionId, userId });
      ...
      const manualApprovalResult = await this.studentVerificationService.uploadDocument(fallbackDocument, userId, true);

      return {
        success: false,
        error: 'Resposta inesperada do servidor - redirecionado para aprovação manual',
        fallbackToManual: true,
        manualApprovalId: manualApprovalResult.documentId,
      };
    } catch (fallbackError) {
      this.logger.error('Failed to create manual approval fallback:', fallbackError);
      return {
        success: false,
