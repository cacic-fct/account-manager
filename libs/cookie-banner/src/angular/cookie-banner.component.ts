        ...
        <div class="cacic-cookie-banner_content">
          ...
          <div class="cacic-cookie-banner_actions">
            <button
              ...
              (click)="accept()"
            >
              <span class="cacic-cookie-banner_accept-icon" aria-hidden="true">
                ✓
              </span>
              <span
                class="cacic-cookie-banner_spinner"
                aria-hidden="true"
              ></span>
...
export class CookieBannerComponent implements OnInit {
  ...
  protected readonly buttonText = computed(
    () => this.config().buttonText ?? 'Prosseguir',
  );
  protected readonly ariaLabel = computed(
    () => this.config().ariaLabel ?? 'Aviso sobre cookies',
  );
  protected readonly bannerClass = computed(() =>
    ['cacic-cookie-banner', this.config().className ?? '']
      .filter(Boolean)
      .join(' '),
  );
